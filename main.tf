...
locals {
  ...
  rules = concat(local.rules_both, local.egress_rules_both, local.egress_rules, local.ingress_rules_both, local.ingress_rules, [])
}
...
resource "openstack_networking_secgroup_rule_v2" "rules" {
  ...
  remote_ip_prefix = lookup(local.rules[count.index], "remote_ip_prefix", null)
  remote_group_id  = lookup(local.rules[count.index], "remote_group_id", null) == "@self" ? local.this_sg_id : lookup(local.rules[count.index], "remote_group_id", null)
}
